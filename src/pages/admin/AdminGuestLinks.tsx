...
const AdminGuestLinks: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const queryClient = useQueryClient();

  const { data: links = [], isLoading } = useQuery({
    queryKey: ['adminLinks'], // using the same cache key is fine, we just filter it
    queryFn: getAllLinks
  });
  ...
  const filteredLinks = links
    .filter(link => link.isGuest) // only guest links
    .filter(link => 
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-surface-mediumgray dark:text-slate-400">
            View and manage anonymous links created by guests.
          </p>
        </div>
        <div className="mt-4 sm:mt-0 relative max-w-sm w-full">
          ...
          <input
            ...
            placeholder="Search guest links..."
            value={searchTerm}
                  ...
                  <tr key={link.id} className="hover:bg-surface-offwhite/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <button
                          onClick={() => {
                            if(window.confirm('Are you sure you want to delete this guest link permanently?')) {
